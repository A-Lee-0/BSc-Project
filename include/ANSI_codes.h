//
// Copied from https://vorbrodt.blog/2019/03/21/ansi-escape-codes/
// Credit to Martin Vorbrodt
//
// Note, to get these to work on windows 10, had to modify registry, i.e. run as admin in cmd: "reg add HKEY_CURRENT_USER\Console /v VirtualTerminalLevel /t REG_DWORD /d 0x00000001 /f"

#ifndef SGCB_ANSI_CODES_H
#define SGCB_ANSI_CODES_H
...
#define ENTRY(name, code) \
    ...
    std::ostream& operator << (std::ostream& os, const name##_opcode&) \
...
#define COLOR_ENTRY(name, code) \
    static struct name##_color {} name; \
    std::ostream& operator << (std::ostream& os, const name##_color&) \
    { os << "\x1b[" #code "m"; return os; }
